...
class FireGento_DynamicCategory_Model_Mysql4_Rule extends Mage_Core_Model_Mysql4_Abstract
{
    ...
    public function getDynamicProductIdsByCategory($categoryId)
    {
        if(!isset($this->_dynamicProductdIdsByCategory[$categoryId])){
            ...
            $select = $rc->select()->from($this->_productCategoryTable, array('product_id'))
    ...
    protected function _saveCategories($productIds, $categoryId)
    {
        ...
        $rc = $this->getReadConnection();
        $select = $rc->select()->from($this->_productCategoryTable)
                    ->where('category_id = ?', $categoryId)
                    ->order('dynamic asc');
    ...
    protected function _getRulesByCategoryIds($categoryIds = null, $storeId = null)
    {
        ...
        $attribute = Mage::getModel('eav/entity_attribute')->loadByCode('catalog_category', self::RULE_ATTRIBUTE_CODE);    
        
        $rc = $this->getReadConnection();
        $select = $rc->select()->from($attribute->getBackendTable(), array('entity_id', 'value'))
